...
angular.module('hellbergApp').factory('Foursquare', ['$http', '$q', 'LOCALE', function($http, $q, LOCALE) {
  var instance = {};
    ...
    get_wikipedia_page(dest.name).then(function(response) {
      ...
      if (res.type === 'redirect') {
        get_wikipedia_page(res.name).then(function(response) {
          var res = parse_wikipedia_page(response);
          console.log('wikidfd.resolve');
          wikidfd.resolve(res);
        });
      } else {
        // res['type'] == 'questions'
        console.log('wikidfd.resolve');
        wikidfd.resolve(res.questions);
      }
    });
